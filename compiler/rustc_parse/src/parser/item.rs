...
impl<'a> DaikonDtraceVisitor<'a> {
    ...
    fn insert_return(
        ...
        .unwrap();
        *exit_counter += 1;
        // FIXME: create methods specialized for common tasks like
    ...
    fn base_impl_item(&mut self) -> Box<Item> {
        ...
        let base_impl_item = self.parser.parse_items_from_source_str(base_impl);
    ...
    fn gen_impl(
        ...
        // FIXME: remove this.
        // match &self.parser.parse_items_from_source_str(struct_as_ret) {
        ...
        let dtrace_print_fields_vec =
            self.build_dtrace_print_fields_vec(&plain_struct, struct_fields);
            ...
            let dtrace_print_xfield = match &get_basic_type(&fields[i].ty.kind, &mut is_ref) {
                RustType::Prim(p_type) => {
                    ...
                    if p_type == "String" || p_type == "str" {
                        ...
                        .unwrap() // FIXME: change this name to involve vec to be clear.
                    }
                }
                RustType::PrimVec(p_type) => {
                    let first_tmp: &str = &daikon_tmp_counter.to_string();
            ...
            let dtrace_field_vec_rec = match &get_basic_type(&fields[i].ty.kind, &mut is_ref) {
                ...
                RustType::Prim(_) => {
                    ...
                    format!(
                        ...
                        .unwrap(),
                        {
                            let tmp_var_name = format!("__daikon_tmp{}", first_tmp);
                            substitute(
                ...
                RustType::UserDef(field_type) => {
                    ...
                    {
                        let tmp_vec_name_str = format!("__daikon_tmp{}", first_tmp);
                        format!(
                            "{}\n{}\n{}",
                            tmp_vec,
                            substitute(
                                ...
                                &FxHashMap::from_iter([
                                    ...
                                    ("tmp_vec_name", tmp_vec_name_str.as_str()),
                                    ("field_name", field_name)
                                ])
                            )
                            .unwrap(),
    ...
    fn build_dtrace_print_fields(&mut self, fields: &mut ThinVec<FieldDef>) -> String {
        let mut dtrace_print_fields: String = DTRACE_PRINT_FIELDS_PROLOGUE.to_string();
                        ...
                        *daikon_tmp_counter += 1;
                        ...
                        let res = format!(
                            ...
                            substitute(
                                DTRACE_VEC_FIELDS,
                                &FxHashMap::from_iter([
                                    ("plain_struct", basic_type.as_str()),
                                    ("tmp_vec_name", tmp_vec_name_str.as_str()),
                                    ("var_name", var_name)
                                ])
                            )
    ...
    pub fn parse_mod(
        ...
        *DO_VISITOR.lock().unwrap() = match &source_file {
        ...
        if *DO_VISITOR.lock().unwrap() {
            ...
            match &self.parse_items_from_source_str(IMPORTS) {
